...
class WorkflowState:
    ...
    def _ensure_directories(self):
        ...
        (self.workspace / ".attest").mkdir(parents=True, exist_ok=True)
        (self.workspace / ".attest" / "artifacts").mkdir(exist_ok=True)
        (self.workspace / ".attest" / "logs").mkdir(exist_ok=True)
    
    @property
    def state_file(self) -> Path:
        """Path to state JSON file."""
        return self.workspace / ".attest" / "state.json"
    
    @property
    def artifacts_dir(self) -> Path:
        """Path to artifacts directory."""
        return self.workspace / ".attest" / "artifacts"
    ...
    def persist(self):
        ...
        state_file = self.workspace / ".attest" / "state.json"
    ...
    def load(cls, workspace: str) -> Optional['WorkflowState']:
        ...
        state_file = Path(workspace) / ".attest" / "state.json"
